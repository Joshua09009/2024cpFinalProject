...
end_bg = pygame.image.load("images\end.jpg").convert_alpha()
end_button = pygame.image.load("images\start_button.png").convert_alpha()
end_bg = pygame.transform.scale(end_bg, (800, 600))
end_button = pygame.transform.scale(end_button, (200, 100))

playing_window = pygame.image.load("images\playing_window.png").convert_alpha()
playing_window = pygame.transform.scale(playing_window, (800, 600))
...
class Note():
    ...
    def note1_coordinate_update(self, time_pass):
        p = time_pass - self.drop_time # 開始墜落後經過的時間。
        # 上面的時間 * 像素每秒 - (目前位置 + 60) = 要增加的座標 (60 是測試出來的緩衝座標)
        self.ycor += pixel_per_second * p - (self.ycor + 60)
        self.xcor -= pixel_per_second * p * 60/565 + (self.xcor - 240)
       
    def note2_coordinate_update(self, time_pass):
        p = time_pass - self.drop_time # 開始墜落後經過的時間。
        # 上面的時間 * 像素每秒 - (目前位置 + 60) = 要增加的座標 (60 是測試出來的緩衝座標)
        self.ycor += pixel_per_second * p - (self.ycor + 60)
        self.xcor -= pixel_per_second * p * 20/565 + (self.xcor - 320)
       
    def note3_coordinate_update(self, time_pass):
        p = time_pass - self.drop_time # 開始墜落後經過的時間。
        # 上面的時間 * 像素每秒 - (目前位置 + 60) = 要增加的座標 (60 是測試出來的緩衝座標)
        self.ycor += pixel_per_second * p - (self.ycor + 60)
        self.xcor += pixel_per_second * p * 20/565 - (self.xcor - 390)
       
    def note4_coordinate_update(self, time_pass):
        p = time_pass - self.drop_time # 開始墜落後經過的時間。
        # 上面的時間 * 像素每秒 - (目前位置 + 60) = 要增加的座標 (60 是測試出來的緩衝座標)
        self.ycor += pixel_per_second * p - (self.ycor + 60)
        self.xcor += pixel_per_second * p * 60/565 - (self.xcor - 470)
...
def background_display(mouse_pos):
    ...
    global start_time
    if started:
        wn.blit(playing_window, (0, 0))
        combo_showing()
        if time_pass > music_time:
            started = False
            waiting = False
            end = True
    elif end:
        wn.blit(end_bg, (0, 0))
        wn.blit(end_button, (100, 200))
        if mouse == "down":
            if mouse_pos[0] > 100 and mouse_pos[0] < 300 and mouse_pos[1] > 200 and mouse_pos[1] < 500:
                end = False
                started = False
                waiting = True
    elif waiting:
...
def draw_back():
    ...
    #pygame.draw.line(wn, (100, 100, 100), (150, 480),(650, 480))    #early
    pygame.draw.line(wn, (100, 100, 100), (150, 500),(650, 500))
    pygame.draw.line(wn, (100, 100, 100), (150, 530),(650, 530))
    #pygame.draw.line(wn, (100, 100, 100), (150, 550),(650, 550))    #late
...
def showingArray_appending(time_pass):
    ...
    global pointer
    coresponding_location = [280, 360, 440, 520]
    ...
    while pointer < len(times_drop) and abs(time_pass - times_drop[pointer]) <= 0.1:
        ...
        print(coresponding_location[notes[pointer]])
...
def note_displaying(time_pass):
    ...
    for one_note in showing_array:
        if one_note.show:
            if one_note.block == pygame.K_d:
                one_note.note1_coordinate_update(time_pass)
                wn.blit(mayo, (one_note.xcor, one_note.ycor))
            if one_note.block == pygame.K_f:
                one_note.note2_coordinate_update(time_pass)
                wn.blit(mayo, (one_note.xcor, one_note.ycor))
            if one_note.block == pygame.K_j:
                one_note.note3_coordinate_update(time_pass)
                wn.blit(mayo, (one_note.xcor, one_note.ycor))
            if one_note.block == pygame.K_k:
                one_note.note4_coordinate_update(time_pass)
                wn.blit(mayo, (one_note.xcor, one_note.ycor))
...
def pygame_events():
    ...
    for event in pygame.event.get():
        ...
        if event.type != pygame.MOUSEBUTTONDOWN:
            mouse = ""
        if event.type == pygame.KEYDOWN and event.key == "p" and not pause:
            pause = True
        if event.type == pygame.KEYDOWN and event.key == "p" and pause:
            pause = False
